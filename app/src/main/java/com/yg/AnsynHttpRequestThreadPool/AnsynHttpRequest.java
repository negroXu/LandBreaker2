...
public class AnsynHttpRequest {
	 ...
	 * @param isShowDialog 是否弹出提示等待�?
	 */
	public static void requestByGet(Context context,
			final Interface_MyThread callBack, String url ,int flag,
...
//			if(data != null && data.length > 0 && listData && TimeCompare.compareHourTime(data[1]) > 1){
		...
		if(isCache && data != null && data.length > 0){ //如果是缓�? 取数据库缓存中数�?
			callBack.Callback_MyThread(data[0], flag);
			return;
		}
		// 异步请求数据
		doAsynRequest(GET, null ,context, callBack, requestUrl, isCache, isShowDialog, flag);
	}
//	/**
//	 *
//	 * 访问网络初始化函�? 支持GET请求方式
...
//		Log.i("httpurl", requestUrl);
//		//判断网络是否可用
//		if (!Network.checkNetWork(context)) { // 如果不可�? 看本地否有数�?
//			if(isCache){ //如果是缓�? 取数据库缓存中数�?
...
//	}
	...
	private static void doAsynRequest(final int sendType,
			...
			final Interface_MyThread callBack, final String url,
	...
	public static void requestByPost(Context context,
			final Interface_MyThread callBack, String url, Map<String, String> map,int requestCode,
			boolean isCache, boolean isShowDialog) {
...
//		requestUrl = requestUrl.substring(0,requestUrl.length()-1);
		Log.i("httpurl", requestUrl);
		// 异步请求数据
		doAsynRequest(POST, map ,context, callBack, url, isCache, isShowDialog, requestCode);
	}
	...
	public static void requestByPost(Context context,String http_head,
			final Interface_MyThread callBack, int url, Map<String, String> map,
			boolean isCache, boolean isShowDialog) {
...
//		requestUrl = requestUrl.substring(0,requestUrl.length()-1);
		...
		if (!Network.checkNetWork(context)) { // 如果不可用 看本地否有数据
			sendBroadcastReceiverMessage(context, R.string.network_show_connectionless);
			callBack.Callback_MyThread(null, url);
			//((BaseActivity) context).checkNetWorkShowLog(1);
			return;
		}
		// 异步请求数据
...
class MyRunnable implements Runnable{
	...
	final Interface_MyThread callBack;
	final String url;
...
//				httpClient.getCookieStore().getCookies().g
			...
			Log.e(AnsynHttpRequest.tag, e.toString());
			if(!isCache){
				AnsynHttpRequest.sendBroadcastReceiverMessage(context, R.string.network_show_error);
			}
			data = null;
		...
		try { // 回调数据
			if(callBack != null){
				callBack.Callback_MyThread(data, intUrl);
...
//			Log.e(AnsynHttpRequest.tag, e.getMessage());
		...
		try{
//			if(isShowDialog) mHandler.sendEmptyMessage(0); // 移除加载对话�?
			return;
		} catch (Exception e) {
			Log.e(AnsynHttpRequest.tag, e.getMessage());
		}
...
//			((Activity)context).removeDialog(1);// 移除加载对话�?
//		};
//	};
}
